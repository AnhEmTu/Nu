...
-- Chức năng hiển thị highlight người chơi
...
game.Players.PlayerRemoving:Connect(function(PlayerRemove)
    if Highlight_Folder:FindFirstChild(PlayerRemove.Name) then
        Highlight_Folder[PlayerRemove.Name]:Destroy()
    end
    if Connection[PlayerRemove] then
        Connection[PlayerRemove]:Disconnect()
    end
end)

-- Tắt và bật hiển thị khi cửa sổ mất hoặc có focus
