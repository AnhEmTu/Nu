...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('extort.lua v5 | %s fps | %s ms'):format(
        math.floor(FPS),
...
getgenv().extort.VerticalPrediction = getgenv().extort.prediction
getgenv().extort.HorizontalPrediction = getgenv().extort.prediction

LeftGroupBox:AddToggle('Enabled', {
    Text = 'Target Lock',
    Default = getgenv().extort.Enabled,
    Callback = function(Value)
        getgenv().extort.Enabled = Value
    end
})
...
LeftGroupBox:AddToggle('NoGroundShot', {
    Text = 'No Ground Shot',
    Default = getgenv().extort.NoGroundShot,
    Callback = function(Value)
        getgenv().extort.NoGroundShot = Value
    end
})

LeftGroupBox:AddToggle('LookAt', {
    Text = 'Look At',
    Default = getgenv().extort.LookAt,
    Callback = function(Value)
        getgenv().extort.LookAt = Value
    end
})

LeftGroupBox:AddToggle('LookAt', {
    Text = 'HighLight',
    Default = getgenv().extort.cham,
    Callback = function(Value)
        getgenv().extort.cham = Value
    end
})
...
DickHead:AddToggle('CamLock', {
    Text = 'Enable CamLock',
    Default = getgenv().extort.Camera,
    Callback = function(Value)
        getgenv().extort.Camera = Value
    end
})


DickHead:AddInput('Smoothness', {
    Default = tostring(getgenv().extort.smoothness),
    Numeric = true,
    Text = 'Camera Smoothness',
    Callback = function(Value)
        getgenv().extort.smoothness = tonumber(Value)
    end
})
...
sigmaox:AddToggle('Auto Air', {
    ...
    Default = getgenv().extort.AutoAir,
    Callback = function(Value)
        getgenv().extort.AutoAir = Value
    end
})

sigmaox:AddInput('ShootDelay', {
    Default = tostring(getgenv().extort.ShootDelay),
    Numeric = true,
    Text = 'Shoot Delay',
    Callback = function(Value)
        getgenv().extort.ShootDelay = tonumber(Value)
    end
})
...
RightGroupBox:AddInput('HorizontalPrediction', {
    Default = tostring(getgenv().extort.prediction),
    Numeric = true,
    Text = 'Prediction',
    Callback = function(Value)
        if not getgenv().extort.AutoPrediction then
            getgenv().extort.prediction = tonumber(Value)
        end
    end
})
RightGroupBox:AddInput('Jump Offset', {
    Default = tostring(getgenv().extort.jumpoffset),
    Text = 'Jump Offset',
    Callback = function(Value)
        getgenv().extort.jumpoffset = Value
    end
})

RightGroupBox:AddToggle('AutoPrediction', {
    Text = 'Auto Prediction',
    Default = getgenv().extort.AutoPrediction,
    Callback = function(Value)
        getgenv().extort.AutoPrediction = Value
    end
})
...
RightGroupBox:AddDropdown('AutoPredMode', {
    ...
    Default = getgenv().extort.AutoPredMode == 'PingBased' and 2 or 1, -- Default to PingBased
    Text = 'Auto Prediction Mode',
    Callback = function(Value)
        getgenv().extort.AutoPredMode = Value
    end
})

RightGroupBox:AddDropdown('SelectedPart', {
    Values = { 'Head', 'LowerTorso', 'UpperTorso', 'HumanoidRootPart' },
    Default = getgenv().extort.SelectedPart == 'Head' and 1 or
              getgenv().extort.SelectedPart == 'LowerTorso' and 2 or
              getgenv().extort.SelectedPart == 'UpperTorso' and 3 or 4,
    Text = 'Target Part',
    Callback = function(Value)
        getgenv().extort.SelectedPart = Value
    end
})
...
ThemeManager:SetFolder('extortTheme')
SaveManager:SetFolder('extortCFG')
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        if getgenv().extort.AntiEnabled then
            if getgenv().extort.AntiLock == "Predbreaker" then
                humanoidRootPart.Velocity = Vector3.new(0, 0, 0)
            elseif getgenv().extort.AntiLock == "Sky" then
                humanoidRootPart.Velocity = Vector3.new(0, 100, 0)
            elseif getgenv().extort.AntiLock == "Ground" then
...
getgenv().extort.LockType = "Namecall"
getgenv().extort.RESOLVER = "MoveDirection"
...
function createCham()
    local settings = getgenv().extort
    if Plr and Plr.Character and Plr.Character:FindFirstChild(settings.SelectedPart) and settings.cham then
        if Plr.Character:FindFirstChild("PlayerCham") then
            Plr.Character.PlayerCham:Destroy()
        end

        local cham = Instance.new("Highlight")
        cham.Name = "PlayerCham"
        cham.Adornee = Plr.Character
        cham.FillColor = Color3.fromRGB(0, 0, 255)
        cham.FillTransparency = Color3.fromRGB(0, 0, 255)
...
function LookAtPlayer(Target)
    ...
    if localHumanoidRootPart then
        if getgenv().extort and getgenv().extort.LookAt then
...
local function toggleLock()
    if enabled then
     if Plr and Plr.Character and Plr.Character:FindFirstChild("PlayerCham") then
            Plr.Character.PlayerCham:Destroy()
        end
   ImageButton.Image = "rbxassetid://284402785"
        enabled = false
        Plr = nil
        destroyTracer()  
        ...
        if Plr then
            ...
            if getgenv().extort.cham then       
                createCham()
            end
            
            if getgenv().extort.TracerEnabled then
                createTracer(game.Players.LocalPlayer, Plr)  
            end
            
            ImageButton.Image = "rbxassetid://284402752"
...
mt.__namecall = newcclosure(function(...)
    ...
    if getgenv().extort.Enabled and getgenv().extort.LockType == "Namecall" then
        if Plr and Plr.Character and method == "FireServer" and (args[2] == remoteInfo.Argument) then
            local selectedPart = getgenv().extort.SelectedPart
            local targetPart = Plr.Character[selectedPart]

            if targetPart then
                local velocity
                if getgenv().extort.ResolverEnabled then
                    if getgenv().extort.RESOLVER == "MoveDirection" then
                        velocity = Plr.Character.Humanoid.MoveDirection * Plr.Character.Humanoid.WalkSpeed
                    elseif getgenv().extort.RESOLVER == "LookVector" then
                        velocity = targetPart.CFrame.LookVector * getgenv().extort.HorizontalPrediction * 1.0
                    else
                ...
                local horizontalPrediction = getgenv().extort.HorizontalPrediction
                local verticalPrediction = getgenv().extort.VerticalPrediction
                local jumpOffset = getgenv().extort.jumpoffset or 0

                if not getgenv().extort.ResolverEnabled then
                    args[3] = Vector3.new(
...
local function updatePredictionValue()
    if getgenv().extort.AutoPrediction then
        local pingValue = Stas.Network.ServerStatsItem["Data Ping"]:GetValueString()
        local split = string.split(pingValue, '(')
        local ping = tonumber(split[1])

        if ping then
            if getgenv().extort.AutoPredMode == "PingBased" then
                for i = 1, #predictionTable do
                    if ping < predictionTable[i][1] then
                        local value = predictionTable[i][2]
                        getgenv().extort.HorizontalPrediction = value
                        getgenv().extort.VerticalPrediction = value
                        break
                    end
                end
            elseif getgenv().extort.AutoPredMode == "Calculation" then
                local calculatedValue = 0.1 + (ping / 1000) * 0.32
                getgenv().extort.HorizontalPrediction = calculatedValue
                getgenv().extort.VerticalPrediction = calculatedValue
            elseif getgenv().extort.AutoPredMode == "AdvanceCalculation" then
                for i = 1, #predictionTable do
                    if ping < predictionTable[i][1] then
                        ...
                        getgenv().extort.HorizontalPrediction = advancePredictionValue
                        getgenv().extort.VerticalPrediction = advancePredictionValue
                        break
                    end
                end
            elseif getgenv().extort.AutoPredMode == "Blatant" then
                for i = 1, #Blatant do
...
RunService.Heartbeat:Connect(function()
    if getgenv().extort.Flick and Plr and Plr.Character then
        ...
        if playerHumanoid and playerHumanoid.Health > 0 then
            ...
            if targetPart and playerHumanoid.FloorMaterial == Enum.Material.Air then
                ...
                local targetPosition = targetPart.Position + targetPart.Velocity * getgenv().extort.VerticalPrediction
...
RunService.Heartbeat:Connect(function()
    if getgenv().extort.Camera and Plr and Plr.Character and getgenv().extort.SelectedPart then
        local camera = Workspace.CurrentCamera
        local selectedPart = getgenv().extort.SelectedPart
        local targetPart = Plr.Character[selectedPart]
        ...
        if targetPart then
            ...
            local jumpOffset = getgenv().extort.jumpoffset or 0
            local fallOffset = getgenv().extort.FallOffset or 0
            ...
            local smoothness = getgenv().extort.smoothness or 0.1 
...
local function ApplyAntiGround()
    ...
    if humanoidRootPart then
        ...
        if not SigmaAir and velocity.Y < -3 then
            if getgenv().extort.NoGroundShot then
                humanoidRootPart.Velocity = Vector3.new(velocity.X, 0, velocity.Z)
            end
        else
            if not getgenv().extort.NoGroundShot then
...
local targetSigm99928 = getgenv().extort.ShootDelay 
...
local function checkTarget()
    if Plr and Plr.Character then
        ...
        if humanoid and humanoidRootPart then
            ...
            if SigmaAir and getgenv().extort.AutoAir then
...
targetSigm99928 = getgenv().extort.ShootDelay

    if getgenv().extort.SilentAim then
